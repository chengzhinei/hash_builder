...
impl HashBuilder {
    ...
    fn update(&mut self, succeeding: &Nibbles) {
        ...
        loop {
            ...
            println!("group_len:{}, preceding_len:{}, common_prefix_len:{}", self.groups.len(), preceding_len, common_prefix_len);
            ...
            if !build_extensions {
                match &self.value {
                    ...
                    HashBuilderValue::Hash(hash) => {
                        println!("in hash_builder. Branch!!!");
            ...
            println!("judge: succeding empty:{}, preceding_exists:{}", succeeding.is_empty(), preceding_exists);
            if !succeeding.is_empty() || preceding_exists {
                println!("ready to store_branch_node!!! current:{:?}", current);
    ...
    fn store_branch_node(&mut self, current: &Nibbles, len: usize, children: Vec<B256>) {
        println!("in store_branch_node. current:{:?}, len:{}", current, len);
        if len > 0 {
        ...
        println!("store_in_db_trie: {}", store_in_db_trie);
        if store_in_db_trie {
            if len > 0 {
                let parent_index = len - 1;
            ...
            if let Some(nodes) = self.updated_branch_nodes.as_mut() {
                println!("insert updated_branch_node. common_prefix:{:?}", common_prefix);
